...
let name_lambda strict arg layout fn =
  ...
  | _ ->
      ...
      let debug_uid = debug_uid_none in
      (* CR sspies: Should we create a debug uid here? *)
      Llet(strict, layout, id, debug_uid, arg, fn id)
...
let name_lambda_list args fn =
  ...
  | (arg, layout) :: rem ->
      ...
      let debug_uid = debug_uid_none in
      (* CR sspies: Should we create a debug uid here? *)
      Llet(Strict, layout, id, debug_uid, arg, name_list (Lvar id :: names) rem) in
  name_list [] args
...
let build_substs update_env ?(freshen_bound_variables = false) s =
  ...
  let bind id duid l =
    ...
    the debug ids. [freshen_bound_variables] is currently only set by
