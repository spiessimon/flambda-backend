  ...
  let bind arg body =
    let newvar, newvar_duid, newarg =
      ...
      | Lvar v -> (v, Lambda.debug_uid_none, arg)
        (* CR sspies: This seems like the kind of place where we should have
...
(* CR sspies: I'm unsure about the debug id handling in this function. *)
let rec lower_bind v v_duid arg_layout arg lam =
  ...
  | Lswitch (ls, ({ sw_consts = [ (i, act) ]; sw_blocks = [] } as sw), loc,
    ...
    when not (approx_present v ls) ->
      Lswitch (ls, { sw with sw_consts = [ (i, lower_bind v v_duid arg_layout arg act) ] },
               loc, kind)
  | Lswitch (ls, ({ sw_consts = []; sw_blocks = [ (i, act) ] } as sw),
             loc, kind)
    when not (approx_present v ls) ->
      Lswitch (ls, { sw with sw_blocks = [ (i, lower_bind v v_duid arg_layout arg act) ] },
               loc, kind)
...
and compile_match_simplified ~scopes value_kind  repr partial ctx
  ...
  | { args = ((Lvar v as arg), str, sort, layout) :: argl } ->
      ...
      combine_handlers value_kind ~scopes repr partial ctx (v, v_duid, str, layout, arg)
        first_match rem
  | _ -> assert false

and combine_handlers ~scopes value_kind repr partial ctx (v, v_duid, str, arg_layout, arg)
    first_match rem =
  let lam, total =
...
let param_to_var (param, sort, layout) =
  ...
  | _ -> (Ident.create_local "*match*", Lambda.debug_uid_none, sort, layout, Some param)
...
let for_multiple_match ~scopes ~return_layout loc paraml mode pat_act_list partial =
  ...
  let paraml =
    List.map (fun (v, _v_duid, sort, layout, _) -> (Lvar v, sort, layout)) v_paraml
  in
