...
let dacc_inside_function context ~outer_dacc ~params ~my_closure ~my_region
  ...
  let denv =
    ...
    | None ->
      ...
      DE.add_variable denv
        (Bound_var.create my_closure Flambda_debug_uid.none NM.normal)
        (T.unknown K.value)
    | Some function_slot -> (
      match
  ...
  let denv =
    ...
    | Some my_region ->
      let my_region =
        Bound_var.create my_region Flambda_debug_uid.none Name_mode.normal
      in
  ...
  let denv =
    ...
    | Some my_ghost_region ->
      let my_ghost_region =
        Bound_var.create my_ghost_region Flambda_debug_uid.none Name_mode.normal
      in
      DE.add_variable denv my_ghost_region (T.unknown K.region)
  in
  let denv =
    let my_depth =
      Bound_var.create my_depth Flambda_debug_uid.none Name_mode.normal
    in
      ...
      ~implicit_params:
        (Bound_parameters.create
           ([ Bound_parameter.create my_closure
                Flambda_kind.With_subkind.any_value Flambda_debug_uid.none;
    ...
    List.mapi
      (fun i kind_with_subkind ->
        BP.create
          ...
          kind_with_subkind Flambda_debug_uid.none (* CR tnowak: verify *))
