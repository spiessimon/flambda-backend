...
let create ~round ~(resolver : resolver)
  ...
  define_variable
    (define_variable t
       (Bound_var.create toplevel_my_region Flambda_debug_uid.none
          (* CR sspies: fix *) Name_mode.normal)
       K.region)
    (Bound_var.create toplevel_my_ghost_region Flambda_debug_uid.none
       (* CR sspies: fix *) Name_mode.normal)
    K.region
...
let define_parameters ~extra t ~params =
  List.fold_left
    (fun t param ->
      ...
      let var =
        Bound_var.create param_var Flambda_debug_uid.none
          (* CR sspies: fix *) Name_mode.normal
      in
