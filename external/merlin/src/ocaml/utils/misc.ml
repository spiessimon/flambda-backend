...
module Colours = struct
  ...
  let pop ppf =
    ...
    | [] | [_] -> fatal_error "Misc.Colours.pop: too many pops"
  ...
  let push ?fg ?bg ppf =
    let current_state =
      match !state_stack with
      | [] -> fatal_error "Misc.Colours.push: empty stack"
      | state :: _ -> state
    in
