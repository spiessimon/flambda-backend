...
struct
  let run param t =
    ...
    | DW_op_bregx { reg_number; offset_in_bytes } ->
      ...
      >>> fun () -> value (V.sleb128 ~comment:"offset in bytes" offset_in_bytes)
    | DW_op_dup | DW_op_drop | DW_op_pick | DW_op_over | DW_op_swap | DW_op_rot
    | DW_op_deref | DW_op_deref_size _ | DW_op_xderef ->
      unit_result
    ...
    | DW_op_neg | DW_op_not | DW_op_or | DW_op_plus ->
      unit_result
    | DW_op_plus_uconst const -> value (V.uleb128 const)
    | DW_op_shl | DW_op_shr | DW_op_shra | DW_op_xor | DW_op_le | DW_op_ge
    | DW_op_eq | DW_op_lt | DW_op_gt | DW_op_ne ->
      unit_result
    | DW_op_skip { num_bytes_forward } ->
      value
        (V.int16 ~comment:"num bytes to jump forward/backward" num_bytes_forward)
    | DW_op_bra { num_bytes_forward } ->
      value
        (V.int16 ~comment:"num bytes forward/backward, if TOS non-zero"
           num_bytes_forward)
    | DW_op_call2 { label; compilation_unit_header_label } ->
      value
        (V.distance_between_labels_16_bit ~comment:"call2 target" ~upper:label
    ...
    | DW_op_implicit_value (Int i) ->
      let buf =
        ...
        | 8 ->
          ...
          buf
        | n ->
          Misc.fatal_errorf "Dwarf_operator: bad Dwarf_arch_sizes.size_int = %d"
            n
      in
      let comment =
        if !Clflags.keep_asm_file
        then Some (Format.asprintf "implicit value %a" Targetint.print i)
        else None
      in
      value
        (V.uleb128 ?comment (Uint64.of_nonnegative_int_exn (Bytes.length buf)))
      >>> fun () -> value (V.string (Bytes.to_string buf))
    ...
    | DW_op_bit_piece { size_in_bits; offset_in_bits } ->
      let size_in_bits = Targetint.nonnegative_to_uint64_exn size_in_bits in
      let offset_in_bits = Targetint.nonnegative_to_uint64_exn offset_in_bits in
      value (V.uleb128 ~comment:"size in bits" size_in_bits) >>> fun () ->
