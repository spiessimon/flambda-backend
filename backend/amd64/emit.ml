...
let to_x86_directive (dir : ND.Directive.t) : X86_ast.asm_line list =
  ...
  (* CR sspies: The [typ] matters only for MASM. The convention (implemented in
     ...
     data section, we use Machine_width_data (QWORD). The two will be emitted
     differently by MASM. Because some code such as the frame tables have moved
     from the data section to the text section (but were previously still
     emitted with QUAD), using the new directives below changes this
     behavior. *)
...
(* All uses of [.extrn] us NEAR as the type. *)
...
let emit_jump_table t =
  ...
  for i = 0 to Array.length t.elems - 1 do
    ...
    (* CR sspies: I think this has the correct semantics. I wonder whether we
       could do something consistent across all of these expressions. *)
    ...
    { efa_code_label =
      ...
      efa_label_rel =
        (fun lbl ofs ->
          ...
          (* CR sspies: On x86 macOS, this changes the name of the variable, but
             should otherwise be semantically equivalent. *)
