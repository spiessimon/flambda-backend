...
let test size =
  ...
  (*   int64x2#   *)
  (****************)
  let eq = int64x2_u_equal in
  let mk_value i = (interleave_low_64 (int64x2_of_int64 (Int64.of_int (i + 0))) (int64x2_of_int64 (Int64.of_int (i + 1)))) in
  (* Create an array of size [size] *)
  let a : int64x2# array = makearray_dynamic size (interleave_low_64 (int64x2_of_int64 0L) (int64x2_of_int64 1L)) in
  (* Fill [a] with distinct values using block indices *)
  for i = 0 to size - 1 do
    Idx_mut.set a (Idx_mut.unsafe_create_into_array i) (mk_value i);
  done;
