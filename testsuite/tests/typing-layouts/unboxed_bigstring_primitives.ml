...
let swap64 x =
  let open Stdlib_upstream_compatible.Int64_u in
  if Sys.big_endian
  then x |> to_int64 |> bswap64 |> of_int64
  else x

let to_int32 = Stdlib_upstream_compatible.Int32_u.to_int32
let to_int64 = Stdlib_upstream_compatible.Int64_u.to_int64
