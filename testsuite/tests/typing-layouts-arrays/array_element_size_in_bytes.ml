...
(* unboxed int64s *)
let check_int64u ~(init : int64_u) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    assert ((element_size * n / bytes_per_word) = (Obj.size (Obj.repr x)));
    if n = 0 then
      Block_checks.check_empty_array_is_uniform ~array_type:"int64_u" (Obj.repr x)
    else begin
      ...
      match Sys.backend_type with
      | Native -> Block_checks.check_mixed_block_scannable_size ~array_type:"int64_u" (Obj.repr x) 0
      | Bytecode | Other _ -> ()
    end
  in
  List.iter check_one array_sizes_to_check

let int64u_array_element_size = size_in_bytes ([||] : int64_u array)
...
(* unboxed float32s *)
let check_float32u ~(init : float32_u) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    ...
    if n = 0 then
      Block_checks.check_empty_array_is_uniform ~array_type:"float32_u" (Obj.repr x)
    else begin
      ...
      | Native ->
        ...
        assert (tag = expected_tag);
        (* Check mixed block has zero scannable fields *)
        Block_checks.check_mixed_block_scannable_size ~array_type:"float32_u" (Obj.repr x) 0
      | Bytecode | Other _ -> ()
    end
  in
  List.iter check_one array_sizes_to_check

let float32u_array_element_size = size_in_bytes ([||] : float32_u array)

let _ = check_float32u ~init:#42.0s ~element_size:float32u_array_element_size

(* unboxed int32s *)
let check_int32u ~(init : int32_u) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    (* These arrays are packed in native code *)
    let padded_n =
    ...
    assert ((element_size * padded_n / bytes_per_word) = (Obj.size (Obj.repr x)));
    if n = 0 then
      Block_checks.check_empty_array_is_uniform ~array_type:"int32_u" (Obj.repr x)
    else begin
      (* Check tag based on actual array length (n) not padded length *)
      let tag = Obj.tag (Obj.repr x) in
      match Sys.backend_type with
      | Native ->
        ...
        assert (tag = expected_tag);
        (* Check mixed block has zero scannable fields *)
        Block_checks.check_mixed_block_scannable_size ~array_type:"int32_u" (Obj.repr x) 0
      | Bytecode | Other _ -> ()
    end
  in
  List.iter check_one array_sizes_to_check

let int32u_array_element_size = size_in_bytes ([||] : int32_u array)
...
let check_int16u ~(init : int16#) ~element_size =
  let check_one n =
    ...
    in
    assert
      ((element_size * padded_n / bytes_per_word) = (Obj.size (Obj.repr x)));
    if n = 0 then
      Block_checks.check_empty_array_is_uniform
        ~array_type:"int16#" (Obj.repr x)
    else begin
      (* Check tag based on actual array length (n) not padded length *)
      let tag = Obj.tag (Obj.repr x) in
      match Sys.backend_type with
      | Native ->
        ...
        let expected_tag =
          ...
          | 1 -> untagged_int16_array_one_tag
          | 2 -> untagged_int16_array_two_tag
          | 3 -> untagged_int16_array_three_tag
          | _ -> assert false
        in
        assert (tag = expected_tag);
        (* Check mixed block has zero scannable fields *)
        Block_checks.check_mixed_block_scannable_size
          ~array_type:"int16#" (Obj.repr x) 0
      | Bytecode | Other _ -> ()
    end
  in
  List.iter check_one array_sizes_to_check

let int16u_array_element_size = size_in_bytes ([||] : int16# array)
    ...
    if n = 0 then
      ...
      | Native ->
        ...
        let expected_tag =
          ...
          | 1 -> untagged_int8_array_one_tag
          | 2 -> untagged_int8_array_two_tag
          | 3 -> untagged_int8_array_three_tag
          | 4 -> untagged_int8_array_four_tag
          | 5 -> untagged_int8_array_five_tag
          | 6 -> untagged_int8_array_six_tag
          | 7 -> untagged_int8_array_seven_tag
          | _ -> assert false
        in
        assert (tag = expected_tag);
        (* Check mixed block has zero scannable fields *)
        Block_checks.check_mixed_block_scannable_size
          ~array_type:"int8#" (Obj.repr x) 0
      | Bytecode | Other _ -> ()
    end
  in
  List.iter check_one array_sizes_to_check
...
let check_ignorable_product1 ~(init : #(int * float32_u * int * int64_u))
      ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    assert ((element_size * n / bytes_per_word) = (Obj.size (Obj.repr x)));
...
let ignorable_product1_array_element_size =
  size_in_bytes ([||] : #(int * float32_u * int * int64_u) array)
...
type t_ignore = #{ x : int; y : #(float# * int32_u); z: int32_u }
...
(* check lack of float32_u packing in unboxed product arrays *)
let check_float32u_pair ~(init : #(float32_u * float32_u)) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
...
(* check lack of int32_u packing in unboxed product arrays *)
let check_int32u_pair ~(init : #(int32_u * int32_u)) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    (* 2 because there are two components in the unboxed product *)
    match Sys.backend_type with
...
(* check lack of int16# packing in unboxed product arrays *)
let check_int16u_pair ~(init : #(int16# * int16#)) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    (* 2 because there are two components in the unboxed product *)
    match Sys.backend_type with
    ...
    (* This product has no scannable fields (only check non-empty arrays) *)
    if n > 0
    then Block_checks.check_mixed_block_scannable_size
           ~array_type:"int16#_pair" (Obj.repr x) 0
  in
  List.iter check_one array_sizes_to_check

let int16u_pair_array_element_size =
  size_in_bytes ([||] : #(int * t_ignore * bool * t_ignore) array)

let _ = check_int16u_pair ~init:#(#1S, #42S)
          ~element_size:int32u_pair_array_element_size

(* check lack of int8# packing in unboxed product arrays *)
let check_int8u_pair ~(init : #(int8# * int8#)) ~element_size =
  let check_one n =
    let x = makearray_dynamic n init in
    (* 2 because there are two components in the unboxed product *)
    match Sys.backend_type with
    ...
    if n > 0
    then Block_checks.check_mixed_block_scannable_size
           ~array_type:"int8#_pair" (Obj.repr x) 0
  in
  List.iter check_one array_sizes_to_check

let int8u_pair_array_element_size =
  size_in_bytes ([||] : #(int * t_ignore * bool * t_ignore) array)

let _ = check_int8u_pair ~init:#(#1s, #42s)
          ~element_size:int32u_pair_array_element_size
