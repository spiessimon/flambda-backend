...
(* This file contains various tests for [int64_u].  It's not an expect test
...
let test1 () =
  ...
  print_int64u "Test 1, twice_minus_five_again" twice_minus_five_again;
...
(**********************************)
(* Test 3: int64_u in closures *)

(* [go]'s closure should haave an [int] (immediate), a [int64_u] (bits64) and a
   [int64 array] (value). *)
...
let test3 () =
  ...
  *)
  ...
  print_int64u "Test 3, 228: " (f3_manyargs ());
  Array.iteri (Printf.printf "  Test 3, step %d: %Ld\n") steps
...
let[@inline never] test4 () =
  ...
  (* partial application to int64_u *)
  let steps = Array.init 10 (fun _ -> 0L) in
  ...
  (* partial application with int64_u remaining *)
  let steps = Array.init 10 (fun _ -> 0L) in
...
let[@inline never] f5 n m =
  let open Int64_u in
  (* Also testing a closure with only int64_u values *)
  let[@inline never] go f =
    f (n + m)
  in
  go
...
(* overapplication to int64_u and non-int64_u args *)
let f6_3 n k = object
...
(*****************************************)
(* Test 7: int64_u and assert false joins *)
