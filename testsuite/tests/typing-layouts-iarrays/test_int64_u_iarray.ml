...
module Int64_u_array0 :
  Gen_u_iarray.S0 with type element_t = int64_u
                   and type ('a : any mod separable) array_t = 'a iarray 
                   and type mutable_t = int64_u array = struct
  type element_t = int64_u

  type ('a : any mod separable) array_t = 'a iarray
  type mutable_t = int64_u array
  ...
  let empty () : int64_u iarray = 
    let m = unsafe_create_mutable 0 in
    freeze m

  let compare_element x y =
    Int64.compare (Int64_u.to_int64 (x ())) (Int64_u.to_int64 (y ()))
end

module Int64_u_array = Gen_u_iarray.Make (Int64_u_array0)

module Int64_u_array_boxed = Test_gen_u_iarray.Make_boxed (struct
    ...
    module I = struct
      ...
      let print f = Printf.printf "%Ld" f
    end
    module E = struct
      let to_boxed x = Int64_u.to_int64 (x ())
      let of_boxed x () = Int64_u.of_int64 x
    end
  end)
module _ = Test_gen_u_iarray.Test (Int64_u_array_boxed)
