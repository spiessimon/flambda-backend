...
module Int32_u_array0 :
  Gen_u_iarray.S0 with type element_t = int32_u
                   and type ('a : any mod separable) array_t = 'a iarray 
                   and type mutable_t = int32_u array = struct
  type element_t = int32_u

  type ('a : any mod separable) array_t = 'a iarray
  type mutable_t = int32_u array
  ...
  let empty () : int32_u iarray = 
    let m = unsafe_create_mutable 0 in
    freeze m

  let compare_element x y =
    Int32.compare (Int32_u.to_int32 (x ())) (Int32_u.to_int32 (y ()))
end

module Int32_u_array = Gen_u_iarray.Make (Int32_u_array0)

module Int32_u_array_boxed = Test_gen_u_iarray.Make_boxed (struct
    ...
    module E = struct
      let to_boxed x = Int32_u.to_int32 (x ())
      let of_boxed x () = Int32_u.of_int32 x
    end
  end)
module _ = Test_gen_u_iarray.Test (Int32_u_array_boxed)
